...
| 405 + `Allow` shared helper (`defineRouteHandlers`/`methodNotAllowed`), incl. correct `OPTIONS` override                                                                                                                                                    | Helper built and tested; applied to 4 representative route files (`health/live`, `api/media`, `api/admin/offers`, `api/admin/offers/{offerId}`) covering GET-only, POST-only, GET+POST, and PATCH-only shapes. Rollout to the remaining ~57 route files is incremental, tracked in the roadmap — an unmigrated route still returns Next's bare default 405 |
| 404 page `robots noindex` metadata                                                                                                                                                                                                                          | Done (ADR-0020)                                                                                                                                                                                                                                                                                                                                            |
| OpenAPI: 307 corrections for the 3 redirect-emitting paths (previously documented as 302); 413/415 wired to the 2 upload paths; `MALFORMED_REQUEST`/`PAYLOAD_TOO_LARGE`/`UNSUPPORTED_MEDIA_TYPE`/`METHOD_NOT_ALLOWED` added to `ProblemDetails.code`'s enum | Done for the paths reachable today (ADR-0020). A shared `components/responses` entry for `MALFORMED_REQUEST`/`METHOD_NOT_ALLOWED` is deferred until they are wired into a real path (adding an unused OpenAPI component fails `redocly lint`'s `no-unused-components` rule) — lands together with the 405 rollout's next batch                             |
