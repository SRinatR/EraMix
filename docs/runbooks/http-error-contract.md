...
| **409 Conflict**                        | `ORDER_STATE_CONFLICT`, `CONCURRENCY_CONFLICT`, `IDEMPOTENCY_CONFLICT`, `SLUG_CONFLICT`, `PUBLIC_ID_CONFLICT` | Optimistic-concurrency version mismatch, an order state-machine transition that is not legal from the current state, a reused `Idempotency-Key` with a different payload, a slug collision with a current/historical route, or a generated `Product.publicId` collision surviving every bounded retry (ADR-0021 — astronomically rare at the 16-character keyspace; `createProduct` retries internally and only lets this reach the delivery boundary after exhausting all attempts).                                                                                                                                                                                                               |
...
- **404** (`apps/web/src/app/[locale]/not-found.tsx`): rendered by the App
  ...
  accidentally-crawled dead link is not indexed — a static `metadata`
  export (`robots: {index: false, follow: false}`) does this, matching the
  `/admin` layout's own established `noindex` pattern. No canonical link
  is emitted for a 404 — there is nothing to canonicalize.
...
| Route-level tests: auth login/callback/download redirects (307, `Location`)                                                                                                                                                                                 | Done (ADR-0020)                                                                                                                                                                                                                                                                                                                                                                                                                     |
| 405 + `Allow` shared helper (`defineRouteHandlers`/`methodNotAllowed`), incl. correct `OPTIONS` override                                                                                                                                                    | Done (Phase C, 2026-08-03) — rolled out to all 61 `apps/web/src/app/{api,health}/**/route.ts` files, including `health/ready` (previously bypassed the shared pipeline entirely with a hand-built, hardcoded-Russian-title 503). Verified live: `next build` registers every route unchanged, the full unit-test suite (532 tests) and a new `health/ready` route test pass. No route file returns Next's bare default 405 anymore. |
| 404 page `robots noindex` metadata                                                                                                                                                                                                                          | Done (ADR-0020)                                                                                                                                                                                                                                                                                                                                                                                                                     |
| OpenAPI: 307 corrections for the 3 redirect-emitting paths (previously documented as 302); 413/415 wired to the 2 upload paths; `MALFORMED_REQUEST`/`PAYLOAD_TOO_LARGE`/`UNSUPPORTED_MEDIA_TYPE`/`METHOD_NOT_ALLOWED` added to `ProblemDetails.code`'s enum | Done (Phase C, 2026-08-03) — `MalformedRequest`/`MethodNotAllowed` `components/responses` entries added; `405` wired into all 68 OpenAPI operations, `400` wired into the 42 that declare a `requestBody`. `redocly lint` passes with zero unused-component warnings.                                                                                                                                                               |
| `PUBLIC_ID_CONFLICT`/409 (ADR-0021 identifier policy)                                                                                                                                                                                                       | Done (2026-08-04) — `PublicIdConflictError` added to the catalogue and `ProblemDetails.code`'s OpenAPI enum; `PrismaProductRepository.create()` disambiguates it from `SLUG_CONFLICT`; a real-Postgres integration test forces both a genuine publicId collision and a sibling sku collision to prove each maps to its correct distinct code.                                                                                       |
